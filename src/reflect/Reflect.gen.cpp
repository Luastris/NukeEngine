...
bool NukeReflectInit() {
	...
	}
	{
		...
		t.methods.push_back(MakeMethod("SetUpscaleMode", &Game::SetUpscaleMode));
		Reflect_SetMethodDoc("Game", "SetUpscaleMode", "--- UPSCALING + FRAME GENERATION (4.2) ---- The game's choice over its cameras' chains: the vendor upscaler (DLSS / FSR / XeSS / FSR 1, Auto = the best this GPU offers), the quality tier, sharpening and frame generation (DLSS-G / FSR FG / XeSS-FG after the upscaler's vendor; the GPU caps the multiplier). Applied live to every game camera (a chain without an \"upscale\" stage gets one), persisted with the window settings. A vendor this GPU cannot do falls to the next honest tier - ActiveUpscaler tells.", "mode");
		t.methods.push_back(MakeMethod("SetUpscaleQuality", &Game::SetUpscaleQuality));
		Reflect_SetMethodDoc("Game", "SetUpscaleQuality", "", "quality");
		t.methods.push_back(MakeMethod("SetUpscaleSharpness", &Game::SetUpscaleSharpness));
		Reflect_SetMethodDoc("Game", "SetUpscaleSharpness", "", "sharpness");
		t.methods.push_back(MakeMethod("SetFrameGeneration", &Game::SetFrameGeneration));
		Reflect_SetMethodDoc("Game", "SetFrameGeneration", "", "multiplier");
		t.methods.push_back(MakeMethod("ResetUpscaling", &Game::ResetUpscaling));
		t.methods.push_back(MakeMethod("UpscalingSet", &Game::UpscalingSet));
		t.methods.push_back(MakeMethod("GetUpscaleMode", &Game::GetUpscaleMode));
		t.methods.push_back(MakeMethod("GetUpscaleQuality", &Game::GetUpscaleQuality));
		t.methods.push_back(MakeMethod("GetUpscaleSharpness", &Game::GetUpscaleSharpness));
		t.methods.push_back(MakeMethod("GetFrameGeneration", &Game::GetFrameGeneration));
		t.methods.push_back(MakeMethod("ActiveUpscaler", &Game::ActiveUpscaler));
		Reflect_SetMethodDoc("Game", "ActiveUpscaler", "What actually runs on the back buffer (the renderer's answer, never Auto):", "");
		t.methods.push_back(MakeMethod("ActiveFrameGenerator", &Game::ActiveFrameGenerator));
		t.methods.push_back(MakeMethod("ActiveFrameGeneration", &Game::ActiveFrameGeneration));
		t.methods.push_back(MakeMethod("PresentedFps", &Game::PresentedFps));
		t.methods.push_back(MakeMethod("UpscaleInfo", &Game::UpscaleInfo));
		t.methods.push_back(MakeMethod("UpscalerAvailable", &Game::UpscalerAvailable));
		Reflect_SetMethodDoc("Game", "UpscalerAvailable", "What this GPU / backend / build offers (for a settings menu to grey out the rest):", "mode");
		t.methods.push_back(MakeMethod("FrameGenerationAvailable", &Game::FrameGenerationAvailable));
		Reflect_SetMethodDoc("Game", "FrameGenerationAvailable", "", "vendor");

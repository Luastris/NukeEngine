...
public:
	...
	// Rotation utilities. Conventions match Transform (glm-backed): angles in DEGREES,
	// euler order XYZ, forward = +Z (Transform::direction() == Rotate({0,0,1})).
	...
	// Rotation looking along `forward` with `up` as the up-hint (both world-space; forward
	// must be non-zero and not parallel to up).

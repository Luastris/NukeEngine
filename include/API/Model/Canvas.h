...
// When a screen-space canvas is drawn relative to post-processing. WorldSpace canvases are
// always part of the scene (WithWorld).
...
// How a screen-space canvas's reference resolution maps onto the render target: Fit = uniform,
// all visible; Stretch = corners to corners; Expand/FitWidth/FitHeight = uniform, may crop.
...
// A 2D layout container for Sprites, independent of the code-driven runtime GUI (NukeGUI).
// Sprite children anchor within its rectangle: WorldSpace renders them as normal world geometry,
// ScreenSpaceOverlay/Camera as reference pixels scaled over the screen or one camera's output.
...
public:
	...
	// Screen-space only: WithWorld = drawn before post effects, AfterPost = on the final image.
	[[nuke::prop(label="Render Queue", enum="WithWorld,AfterPost")]] CanvasQueue renderQueue = CanvasQueue::AfterPost;
	// WorldSpace: rectangle size in WORLD units. ScreenSpace: reference resolution in PIXELS.
	...
	// Screen-space: how many reference PIXELS one world unit of a child covers. The editor's
	// world-plane preview derives from this too (1 ref px = 1/ppu world units), so children
	// manipulate exactly 1:1 with the world — deliberately NO separate preview-scale knob.
	...
	// Screen-space: when set, ONLY that camera renders this canvas. Null = every game camera.
	// For AtomRef props the asset= hint is the picker FILTER: only atoms with that component.

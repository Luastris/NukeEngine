...
cbuffer GIPassCB
{
    ...
    float4 g_GIRot;
    float4 g_GIMisc;   // x = max ray distance
};
...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    if (c.a >= 0.999 && g_SkyParams.y < 0.5) rad = g_Ambient.rgb;   // miss with the sky off: the clear colour is not light
    g_RayData[(uint)g_GIPass.y * rays + tid.x] = float4(rad, max(d, 0.01));
}

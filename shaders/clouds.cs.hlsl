...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    float3 d   = normalize(wp.xyz / wp.w - o);
    ...
    float3 fwd = normalize(fc.xyz / fc.w - o);   // the camera's forward: view depth -> distance along this ray
    ...
    if (CloudSegment(o, d, min(g_ClLayer.z, sceneDist), tA, tB))
    {
        ...
        [loop] for (int i = 0; i < N * 2 && t < tB; ++i)
        {
            ...
            if (dens > 0.0)
            {
                ...
                float3 S     = (CloudSunLight(tau, mu) + CloudAmbient(hf)) * sigma;   // single-scatter albedo ~1
